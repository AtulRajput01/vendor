...
const AuthList = () => {
  ...
  const fetchAuths = async () => {
    try {
      const response = await axios.get('http://54.71.141.115:3002/auths/');
  ...
  const handleAddAuth = async () => {
    try {
      const response = await axios.post('http://54.71.141.115:3002/auths/website', { username, email, password });
  ...
  const handleDeleteAuth = async (id) => {
    try {
      await axios.delete(`https://54.244.180.151
