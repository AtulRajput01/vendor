...
const Shops = () => {
  ...
  const handleDelete = async (id) => {
    await axios.delete(`http://www.taxidermyvendor.hunt30.com/api/ShopDetails/deleteShop/${id}`);
    fetchShop();
  };
          ...
          {shops.map((shop, index) => (
            <CTableRow key={shop.id}>
              ...
              <CTableDataCell style={{ textAlign: "center" }}>
                {shop.shopLogo ? (
                  <img src={`http://www.taxidermyvendor.hunt30.com/${shop.shopLogo}`} alt="Shop Logo" style={{ width: '50px', height: '50px' }} />
                ) : "null"}
              </CTableDataCell>
              <CTableDataCell>
