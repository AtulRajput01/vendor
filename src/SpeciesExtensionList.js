...
const SpeciesExtensionList = () => {
  ...
  const fetchSpecies = async (shopId) => {
    const response = await axios.post(`http://54.71.141.115:3002/species/getSpecies/${shopId}`);
    setSpecies(response.data.data)
  };
  const fetchExtension = async (shopId) => {
    const response = await axios.post(`http://54.71.141.115:3002/Extension/getByShop/${shopId}`);
    setExtensions(response.data.data)
  };
  const deleteExtension = async (id) => {
    const response = await axios.delete(`http://54.71.141.115:3002/Extension/deleteExten/${id}`);
    fetchExtension(shopId);
  }
  ...
  const handleEditSubmit = async () => {
    const apiUrl = selectedItem.speciesName
      ? `http://54.71.141.115:3002/species/species/${selectedItem._id}`
      : `http://54.71.141.115:3002/Extension/editeExten/${selectedItem._id}`;
  ...
  const deleteSpecies = async (id) => {
    const response = await axios.delete(`http://54.71.141.115:3002/species/deleteSpecies/${id}`);
    fetchSpecies(shopId);
  }
                ...
                species.map((specie, index) => (
                  <CTableRow key={specie._id}>
                    ...
                    <CTableDataCell style={{ textAlign: "center" }}>
                      {specie.speciesImage ? (
                        <img src={`http://54.71.141.115:3002/${specie.speciesImage}`} alt="Species" style={{ width: '50px', height: '50px' }} />
                      ) : "No Image"}
                    </CTableDataCell>
              ...
              {extensions.length === 0 ? (
                ...
                extensions.map((extension, index) => (
                  <CTableRow key={extension._id}>
                    ...
                    <CTableDataCell style={{ textAlign: "center" }}>
                      {extension.image ? (
                        <img src={`http://54.71.141.115:3002/${extension.image}`} alt="Extension" style={{ width: '50px', height: '50px' }} />
                      ) : "No Image"}
                    </CTableDataCell>
