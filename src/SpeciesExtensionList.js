...
const SpeciesExtensionList = () => {
  ...
  const fetchSpecies = async (shopId) => {
    const response = await axios.post(`https://www.taxidermyadmin.hunt30.com/api/species/getSpecies/${shopId}`);
    setSpecies(response.data.data)
  };
  const fetchExtension = async (shopId) => {
    const response = await axios.post(`https://www.taxidermyadmin.hunt30.com/api/Extension/getByShop/${shopId}`);
    setExtensions(response.data.data)
  };
  const deleteExtension = async (id) => {
    const response = await axios.delete(`https://www.taxidermyadmin.hunt30.com/api/Extension/deleteExten/${id}`);
    fetchExtension(shopId);
  }
  ...
  const handleEditSubmit = async () => {
    const apiUrl = selectedItem.speciesName
      ? `https://www.taxidermyadmin.hunt30.com/api/species/species/${selectedItem._id}`
      : `https://www.taxidermyadmin.hunt30.com/api/Extension/editeExten/${selectedItem._id}`;
  ...
  const deleteSpecies = async (id) => {
    const response = await axios.delete(`https://www.taxidermyadmin.hunt30.com/api/species/deleteSpecies/${id}`);
    fetchSpecies(shopId);
  }
                ...
                species.map((specie, index) => (
                  <CTableRow key={specie._id}>
                    ...
                    <CTableDataCell style={{ textAlign: "center" }}>
                      {specie.speciesImage ? (
                        <img src={`https://www.taxidermyadmin.hunt30.com/${specie.speciesImage}`} alt="Species" style={{ width: '50px', height: '50px' }} />
                      ) : "No Image"}
                    </CTableDataCell>
              ...
              {extensions.length === 0 ? (
                ...
                extensions.map((extension, index) => (
                  <CTableRow key={extension._id}>
                    ...
                    <CTableDataCell style={{ textAlign: "center" }}>
                      {extension.image ? (
                        <img src={`https://www.taxidermyadmin.hunt30.com/${extension.image}`} alt="Extension" style={{ width: '50px', height: '50px' }} />
                      ) : "No Image"}
                    </CTableDataCell>
