...
const OrderStatusTrack = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      const response = await axios.get('http://54.244.180.151:3002/api/OrderDetails/status/ACCEPT');
  ...
  const handleUpdateTrackStatus = async () => {
    try {
      await axios.put(`http://54.244.180.151:3002/api/OrderStatusTrack/${selectedOrder._id}`, { trackStatus: newTrackStatus });
  ...
  const handleDeleteOrder = async (orderId) => {
    try {
      await axios.delete(`http://54.244.180.151:3002/api/OrderDetails/${orderId}`);
