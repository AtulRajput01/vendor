...
const OrderStatusTrack = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      const response = await axios.get('http://www.taxidermyvendor.hunt30.com/api/OrderDetails/status/ACCEPT');
  ...
  const handleUpdateTrackStatus = async () => {
    try {
      await axios.put(`http://www.taxidermyvendor.hunt30.com/api/OrderStatusTrack/${selectedOrder._id}`, { trackStatus: newTrackStatus });
  ...
  const handleDeleteOrder = async (orderId) => {
    try {
      await axios.delete(`http://www.taxidermyvendor.hunt30.com/api/OrderDetails/${orderId}`);
      ...
      <CCardHeader style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <CDropdown className="float-right">
          ...
          <CDropdownMenu>
            ...
            <CDropdownItem onClick={() => handleFilterChange('ORDER CONFIRMED')}>Order Confirmed</CDropdownItem>
            <CDropdownItem onClick={() => handleFilterChange('PENDING')}>Pending</CDropdownItem>
            <CDropdownItem onClick={() => handleFilterChange('ORDER BEING PROCESSED')}>Order Being Processed</CDropdownItem>
            <CDropdownItem onClick={() => handleFilterChange('ORDER DISPATCHED')}>Order Dispatched</CDropdownItem>
            <CDropdownItem onClick={() => handleFilterChange('ORDER ON ITS WAY')}>Order On Its Way</CDropdownItem>
            <CDropdownItem onClick={() => handleFilterChange('ORDER RECEIVED')}>Order Received</CDropdownItem>
          </CDropdownMenu>
        </CDropdown>
      </CCardHeader>
      <CCardBody>
